...
def lpdtree():
    ...
    for i in items:
        tree = rtr.TREES["A"].tree_
        for i in range(len(tree.value)):
            if tree.children_left[i] != -1:
            # a decision node
#        print i, "decision node: feature "+str(tree.feature[i]) + " <= " + "constant " + str(tree.threshold[i])
            else:
            # a leaf node
#        print i, "leaf node: constant = " + str(tree.value[i][0][0]), " path = (" + find_path(tree, i) + " )"
